    ...
    <div className="w-full bg-white/70 backdrop-blur-xl saturate-[160%] border border-white/50 p-2.5 sm:p-3 rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.6)] relative overflow-hidden">
      ...
      <div className="sm:hidden flex items-center justify-between gap-2 relative z-10">
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-6 h-6 rounded-lg bg-emerald-600 text-white flex items-center justify-center text-xs font-black shrink-0 shadow-xs">
            {currentStep}
          </div>
          <div className="flex flex-col min-w-0">
            <span className="text-[8.5px] font-extrabold text-emerald-700 uppercase tracking-wider leading-none">
        ...
        <div className="flex items-center gap-1 shrink-0">
          {steps.map((step) => {
            ...
            const isCompleted = currentStep > step.number || (stepValidations && stepValidations[step.number]);
        ...
        {steps.map((step) => {
          ...
          return (
            <button
              ...
              disabled={!canClick && !isActive}
              onClick={() => canClick && onStepClick(step.number)}
              className={`flex items-center gap-2.5 p-2 rounded-xl transition-all text-left border ${
                isActive
                  ? 'bg-emerald-600 text-white border-emerald-600 shadow-md shadow-emerald-600/20 ring-2 ring-emerald-600/15 scale-[1.01]'
                  : isCompleted
                  ? 'bg-emerald-500/10 border-emerald-200 text-emerald-800 hover:bg-emerald-500/20 cursor-pointer'
                  : canClick
            ...
            >
              ...
              <div className="flex flex-col min-w-0">
                <span
                  className={`text-[8.5px] font-extrabold uppercase tracking-wider line-clamp-1 ${
